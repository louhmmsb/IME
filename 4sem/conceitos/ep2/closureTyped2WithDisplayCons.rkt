...
(define (equal* [l : Value] [r : Value]) : Value
  (cond
    ...
    [(and (cellV? l) (cellV? r)) (if (equal? (equal* (unbox (cellV-first l)) (unbox (cellV-first r))) (numV 1)) (equal* (unbox (cellV-second l)) (unbox (cellV-second r))) (numV 0))]
...
(define (interp [a : ExprC] [env : Env]) : Value
  (type-case ExprC a
    ...
    [carC   (exp) (let ([xaica (cellV-first (strict (interp exp env)))])
                    (begin (set-box! xaica (strict (unbox xaica)))
                         (unbox xaica)))]
    ;[cdrC  (exp) (cellV-second (strict (interp exp env)))]
    [cdrC  (exp) (let ([xaica (cellV-second (strict (interp exp env)))])
                   (begin (set-box! xaica (strict (unbox xaica)))
                          (unbox xaica)))]
    ;Display values
...
(interpS '(letrec ((displayn (lambda n (lambda l
            ...
            (letrec ((ones (cons 1 ones)))
              (call (call displayn 20) (call (call mapmeu (lambda x (* 7 x))) ones))
                  ))))