...
{
    stackP percorre = iniPosfixa->prox, kill, proximo, resultado;
	...
	case OPER_IGUAL:
	    pop1 = stackPopv(stack);
	    pop2 = stackPopv(stack);
	    pop2->valor.vFloat = (pop1->valor.vFloat == pop2->valor.vFloat);
	    kill = percorre;
	    break;
	case OPER_DIFERENTE:
	    pop1 = stackPopv(stack);
	...
	case OPER_MAIOR_IGUAL:
	    pop1 = stackPopv(stack);
	    pop2 = stackPopv(stack);
	    pop2->valor.vFloat = (pop2->valor.vFloat >= pop1->valor.vFloat);
	    kill = percorre;
	    break;
	...
	case OPER_EXPONENCIACAO:
	    pop1 = stackPopv(stack);
	    pop2 = stackPopv(stack);
	    pop2->valor.vFloat = (pow(pop2->valor.vFloat, pop1->valor.vFloat));
	    kill = percorre;
	    break;
	case OPER_DIVISAO_INT:
	    pop1 = stackPopv(stack);
	    pop2 = stackPopv(stack);
	    pop2->valor.vFloat = (((int)pop2->valor.vFloat)/((int)pop1->valor.vFloat));
	    kill = percorre;
	    break;
	case OPER_MAIOR:
	    pop1 = stackPopv(stack);
	...
	case OPER_RESTO_DIVISAO:
	    pop1 = stackPopv(stack);
	    pop2 = stackPopv(stack);
	    pop2->valor.vFloat = (fmod(pop2->valor.vFloat, pop1->valor.vFloat));
	    kill = percorre;
	    break;
	...
	case OPER_MENOS_UNARIO:
	    pop2 = stackPopv(stack);
	    pop2->valor.vFloat = -(pop2->valor.vFloat);
	    kill = percorre;
	    break;
	...
	case OPER_ATRIBUICAO:
	    pop1 = stackPopv(stack);
	    pop2 = stackPop(stack);
	    setValorST(pop2->valor.pStr, pop1);
	    pop2->categoria = FLOAT;
	    pop2->valor.vFloat = pop1->valor.vFloat;
	    kill = percorre;
	    break;
	case FLOAT:
	case ID:
	...
	if(pop1 != NULL) freeObjeto(pop1);
	percorre = proximo;
	if (kill != NULL){
	    stackPush(pop2, stack);
	    freeObjeto(kill);
	}
    }
    iniPosfixa->prox = NULL;
    percorre = proximo;
    resultado = stackPopv(stack);
    stackFree(stack);
    return resultado;
}

stackP stackPopv(stackP stack){
    stackP retorno, kill;
    if ((stackTop(stack))->categoria == ID){
	retorno = getValorST((stackTop(stack))->valor.pStr);
	kill = stackPop(stack);
	free(kill);
    }
    else retorno = stackPop(stack);
    return retorno;
}
