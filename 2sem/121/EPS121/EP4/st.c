...
getValorST(char *nomeVar)
{
    ...
    achei = mallocSafe(sizeof *achei);
    achei->categoria = FLOAT;
    achei->valor.vFloat = var->valorVar.vFloat;
    achei->prox = NULL;
    return achei;
}
...
setValorST(char *nomeVar, CelObjeto *pValor)
{
    ...
    var  = endVarST(nomeVar);
    if (var == NULL){
	var = mallocSafe(sizeof *var);
	var->nomeVar = mallocSafe((strlen(nomeVar)+1)*(sizeof *(var->nomeVar)));
	strncpy(var->nomeVar, nomeVar, strlen(nomeVar)+1);
...
showST()
{
    CelST *percorre = ini->proxVar;
    printf("==========================\nTabela de simbolos\n'nome': valor\n. . . . . . . . . . . . . .\n");
    while(percorre != NULL){
	printf("'%s': %g\n", percorre->nomeVar, percorre->valorVar.vFloat);
	percorre = percorre->proxVar;
    }
}
...
{
    CelST *percorre = ini->proxVar;
    while(percorre != NULL && strcmp(nomeVar, percorre->nomeVar) != 0) percorre = percorre->proxVar;
    return percorre;
}
