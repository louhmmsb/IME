...
mostreMelhoresFilmes(ListaFilmes *lst)
{
    ...
    float X;
    Filme *percorre = lst->cab->prox;
    printf("Quantos filmes você quer que sejam mostrados: ");
    scanf("%d", &N);
    printf("Nota limite: ");
    scanf("%f", &X);
    printf("Numero minimo de votos: ");
    scanf("%d", &V);
    while(percorre != lst->cab && (percorre->nota)>X) percorre = percorre->prox;
    while(percorre != lst->cab && N>0){
	if((percorre->votos)>=V){
	    N--;
	    mostreFilme(percorre);
	}
    ...
    if(N>0) printf("Esses são todos os filmes dentro das condições pedidas");
}
...
mostrePioresFilmes(ListaFilmes *lst)
{
    ...
    printf("Numero minimo de votos: ");
    scanf("%d", &V);
    while(percorre != lst->cab && (percorre->nota)<X) percorre = percorre->ant;
    while(percorre != lst->cab && N>0){
	if((percorre->votos)>=V){
	    N--;
	    mostreFilme(percorre);
