...
crieListaFilmes()
{
    ...
    lista->cab->nome = mallocSafe(sizeof(char));
    lista->cab->ant = lista->cab;
...
{
    Filme *percorre = lst->cab->prox;
    while(percorre != lst->cab){
	percorre = percorre->prox;
	libereFilme(percorre->ant);
    }
    libereFilme(percorre);
    free(lst);
}
 ...
 *  CHECAR PRA VER SE FUNCIONA MESMO DEPOIS
 */
void 
...
contemFilme(ListaFilmes *lst, Filme *flm)
{
    Filme *percorre = lst->cab->prox;
    while(percorre != lst->cab){
	if(!strcmp((percorre->nome), (flm->nome)) && percorre->nota == flm->nota && percorre->ano == flm->ano) return TRUE;
    }
    return FALSE;
}
...
{
    Filme *percorre = lst->cab->prox;
    while(percorre != flm) percorre = percorre->prox;
...
mergeSortFilmes(ListaFilmes *lst, Criterio criterio)
{
    ...
    int tamanholst1, tamanholst2;
    Filme *aux1 = lst->cab->prox, *aux2 = aux1->prox;
    if(lst->nFilmes <= 1) return;
    lst1 = crieListaFilmes();
    lst2 = crieListaFilmes();
    tamanholst1 = (lst->nFilmes)/2;
    tamanholst2 = (lst->nFilmes)-tamanholst1;
    while(lst1->nFilmes < tamanholst1){
	insiraFilme(lst1, aux1);
	aux1 = aux2;
	aux2 = aux2->prox;
    }
    while(lst2->nFilmes < tamanholst2){
	insiraFilme(lst2, aux1);
	aux1 = aux2;
	aux2 = aux2->prox;
    }
    ...
    lst->cab->ant = lst->cab;
    intercala(lst, lst1, lst2, criterio);
}

/*----------------------------------------------------------------------
 * Funçao para intercalar as listas do mergeSort
 * ---------------------------------------------------------------------
 */
void
intercala(ListaFilmes *lst, ListaFilmes *lst1, ListaFilmes *lst2, Criterio criterio)
{
    Filme *p, *q;
    if(criterio == NOTA){
	p = lst1->cab->prox;
	...
	while(p != lst1->cab && q != lst2->cab){
	    if(q->nota > p->nota){
		q = q->prox;
		insiraFilme(lst, q->ant);
	    }
	    else{
		p = p->prox;
		insiraFilme(lst, p->ant);
	    }
	}
	if(p == lst1->cab){
	    while(q != lst2->cab){
...
Filme
*buscaFilme(ListaFilmes *lst){
    char nome[256], resposta;
    int tamanho;
    Filme *percorre = lst->cab->prox;
    printf("Digite uma parte do nome do filme: ");
    leiaString(nome, 256);
    tamanho = strlen(nome);
    while(percorre != lst->cab){
	if(achePalavra((unsigned char *)nome, tamanho, (unsigned char *)percorre->nome, strlen(percorre->nome))){
	    printf("Era este o filme que voce queria?[s,n,x]\n");
	    mostreFilme(percorre);
	    resposta =  getchar();
	    if(resposta == 's') return percorre;
	    if(resposta == 'n') continue;
	    if(resposta == 'x') return NULL;
	}
	percorre = percorre->prox;
    }
    if(percorre == lst->cab) printf("Seu filme nao esta na lista");
    return NULL;
}
